...
function TicTacToe() {
    ...
    const reset = () => {
        setCount(0);
        const initialData = ["", "", "", "", "", "", "", "", ""];
        titleRef.current.innerHTML = `2-Player Tic Tac Toe Game made <span>React JS</span>`;
        setLock(false);
        setData(initialData);
        clickAudio.play();
    }
    ...
    const won = (winner) => {
        setLock(true);
        if(winner === 'x'){
            titleRef.current.innerHTML = `Congragulations player <span>X</span>`
            setXscore(xscore+1);
        }else if(winner === 'o'){
            titleRef.current.innerHTML = `Congragulations player <span>O</span>`
            setYscore(yscore+1);
        }
        wonAudio.play();
    }
